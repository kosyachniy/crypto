...
def bots():
	while True:
		with db:
			for i in db.execute("SELECT * FROM operations WHERE act=1"):
				cur, exc, buy, per, chat, id, time = i[2], i[3], i[4], i[5], i[8], i[9], i[10]
				db.execute("UPDATE operations SET act=0 WHERE id=(?)", (i[0],))
